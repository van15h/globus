...
public class Reise {
  ...
  public static void generate(Connection connection) {
    try {
		...
		System.out.println("DataImport.Reise connected!");
    } catch (Exception e) {
	...
	for (int i = 1; i <= 1000; i++) {
		...
		try {
			...
			preparedStatement.setString(2, names[ DataImport.Main.random(1, 10) - 1 ]);
			preparedStatement.setDate(3, java.sql.Date.valueOf(today));
			preparedStatement.setInt(4, DataImport.Main.random(1, 30));
			preparedStatement.setInt(5, DataImport.Main.random(10, 1000));
